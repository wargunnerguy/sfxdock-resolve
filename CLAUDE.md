...
| 2026-07-20 | Result metadata is user-configurable (Settings > Result info toggles: Quality/Duration/Source/Uploader; quality+duration+source on, uploader off). Each provider fills a quality field: Freesound rich (bitrate/kHz/bit-depth), local WAV/AIFF read from headers (schema v3), Jamendo just MP3 (API exposes no per-track quality) | Maintainer request |
| 2026-07-20 | Phase 6 OAuth2 (Freesound): authorization-code + loopback redirect (127.0.0.1:8910/callback); tokens encrypted via Electron safeStorage (Windows DPAPI); auto-refresh before 24h expiry; downloads send Bearer token; Login required badge flips to Free once connected. HTTP flow verified correct (authorize 302→login, token endpoint accepts creds). Full interactive connect BLOCKED on the maintainer's Freesound account showing "no API access" — existing key still works for search, but OAuth needs a manageable/approved credential to register the callback URL | Phase 6 finding |
